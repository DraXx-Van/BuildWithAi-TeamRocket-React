  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card flex-col gap-14" style={{ 
        ...
        {incident.status === 'resolved' && incident.assignedTo ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ 
              ...
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: '#10b981', letterSpacing: 2, marginBottom: 8, fontWeight: 800 }}>UNIT DEPLOYED (MISSION COMPLETE)</div>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 700, color: 'white' }}>{incident.assignedTo}</div>
            </div>
            <div style={{ 
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ 
              ...
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: '#f59e0b', letterSpacing: 2, marginBottom: 8, fontWeight: 800 }}>ACTIVE UNIT DEPLOYED</div>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 700, color: 'white' }}>{incident.assignedTo}</div>
            </div>
            <div style={{ 
              ...
              borderRadius: 10, textAlign: 'center', fontFamily: 'var(--font-mono)', 
              fontSize: 10, color: '#f59e0b', fontWeight: 800, letterSpacing: 1.5 
            }}>
              ↻ UNIT EN ROUTE / ON SCENE
            </div>
          </div>
        ) : isThisIncident && dispatchResult && incident.status !== 'resolved' ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ 
        ...
        {incident.status !== 'dispatched' && incident.status !== 'resolved' && !confirmed && !manualConfirmed && (
          <>
            <button
      ...
      {/* Resolution */}
      <div className="card flex-col gap-14" style={{ 
        background: incident.status === 'resolved' ? 'rgba(16,185,129,0.05)' : 'rgba(20,20,25,0.4)', 
        backdropFilter: 'blur(20px)',
        border: `1px solid ${incident.status === 'resolved' ? 'rgba(16,185,129,0.2)' : 'rgba(255,255,255,0.05)'}` 
      }}>
        <div className="step-label">
          <span className="step-num" style={{ background: incident.status === 'resolved' ? 'rgba(16,185,129,0.2)' : 'rgba(239, 68, 68, 0.1)' }}>
            <Zap size={14} color={incident.status === 'resolved' ? '#10b981' : '#ef4444'} />
          </span>
          INCIDENT CONTROL
        </div>
        ...
        {incident.status === 'resolved' ? (
          <div style={{ 
            ...
            borderRadius: 10, fontWeight: 800, fontSize: 11, textAlign: 'center',
            letterSpacing: 1.5 
          }}>
            ✓ CASE CLOSED
          </div>
