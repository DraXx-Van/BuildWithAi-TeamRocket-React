...
// ═══════════════════════════════════════════════════════════════════════════════
// AUTO-SEED — Populate Firestore rooms from local GeoJSON if empty
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Convert a GeoJSON polygon (array of [x,y] coords in pixel space) to
 * percentage values relative to the floor viewport.
 * floor1 viewport: 680 x 400
 */
function geoJsonFeatureToRoom(feature, viewportW = 680, viewportH = 400) {
  ...
  return {
    ...
    name: p.name,
    type: p.type || 'room',
    xPercent: minX / viewportW,
    yPercent: minY / viewportH,
    widthPercent: (maxX - minX) / viewportW,
    heightPercent: (maxY - minY) / viewportH,
    capacity: p.capacity || 2,
  };
}

/**
 * Seeds Firestore with default floor rooms from local GeoJSON if no rooms exist.
 * Also ensures the floor-level document exists so getFullFloorData works.
 * Call once during app init.
 */
export async function seedFloorRoomsIfEmpty(hotelId = 'hotel_default') {
  try {
    // Dynamically import the GeoJSON so it's not in the critical path
    const { default: floor1GeoJSON } = await import('../data/floorplans/floor1.geojson.js');
    const floorLevel = floor1GeoJSON.metadata.floorLevel; // 1

    // Ensure the floor-level document exists
    const floorDocRef = doc(db, `hotels/${hotelId}/floors`, `level_${floorLevel}`);
    const floorSnap = await getDoc(floorDocRef);
    if (!floorSnap.exists()) {
      await setDoc(floorDocRef, {
        floorLevel: Number(floorLevel),
        label: floor1GeoJSON.metadata.label || `Floor ${floorLevel}`,
        uploadedAt: serverTimestamp(),
        imageUrl: null, // no image — rendered as pure canvas
      });
      console.log(`[SEED] Created floor level_${floorLevel} document.`);
    }

    // Check if rooms already exist
    const roomsCollRef = collection(db, `hotels/${hotelId}/floors/level_${floorLevel}/rooms`);
    const existingSnap = await getDocs(roomsCollRef);
    if (!existingSnap.empty) {
      console.log(`[SEED] Floor ${floorLevel} already has ${existingSnap.size} rooms — skipping seed.`);
      return;
    }

    // Seed from GeoJSON features
    const vp = floor1GeoJSON.metadata.viewport;
    const seedPromises = floor1GeoJSON.features.map(async (feature) => {
      const room = geoJsonFeatureToRoom(feature, vp.width, vp.height);
      if (!room) return;
      await saveRoomCoordinate(hotelId, floorLevel, room);
    });
    await Promise.all(seedPromises);
    console.log(`[SEED] Seeded ${floor1GeoJSON.features.length} rooms into floor level_${floorLevel}.`);
  } catch (err) {
    console.warn('[SEED] Could not seed floor rooms:', err.message);
  }
}
