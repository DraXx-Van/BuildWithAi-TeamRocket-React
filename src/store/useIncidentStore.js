...
import {
  ...
  updateIncident, streamEvacuationStatus, triggerEvacuation, releaseStaff
} from '../services/firebaseService';
import { getHotelFloors, getFullFloorData, seedFloorRoomsIfEmpty } from '../services/floorPlanAdminService';
...
const useIncidentStore = create((set, get) => ({
  ...
  init: async () => {
    ...
    // Seed default floor room coordinates if Firestore is empty
    await seedFloorRoomsIfEmpty('hotel_default');
  ...
  processIncidentData: async (transcript, imageBase64 = null, reporterName = null, manualLocation = null) => {
    ...
    try {
      ...
      const location = manualLocation || result.location || 'Unknown Location';
  ...
  confirmDispatch: async () => {
    ...
    set({ dispatchResult: null, dispatchIncidentId: null });
  },
  ...
  resolveIncident: async (id) => {
    const { liveIncidents } = get();
    const incident = liveIncidents.find(i => i.id === id);
    if (incident && incident.assignedTo) {
      try {
        await releaseStaff(incident.assignedStaffId || null, incident.assignedTo);
      } catch (e) {
        console.error('[STORE] Failed to release staff:', e);
      }
    }
